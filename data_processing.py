...
Core data logic for the UIDAI Demographic Volatility Monitoring Platform.

Responsibilities:
- Clean and reshape raw UIDAI aggregated data
- Compute demographic volatility signals
- Generate state-level and national summaries
- Keep all math and transformations out of app.py
"""

import pandas as pd
from utils import classify_volatility
...
def process_uidai_data(df):
    ...
    Input:
        df : pandas DataFrame (raw aggregated UIDAI CSV)
    Output:
        monthly      : time-series volatility data
        latest       : latest snapshot (district + age group)
        state_risk   : state-level volatility summary
        age_summary  : age-wise state summary
        trend        : national volatility trend
    """
